...
inside_area_z2 <- l$fully_included_z2[z1 %in% area_z1_at_risk,]$z2
...
polygone_carreau_interne <-
  carreaux_to_polygon(data.frame(carreau = inside_area_z2), var_carreau = "carreau") %>%
  sf::st_transform(crs = 4326)
...
inside_geom_z2 <- polygone_carreau_interne %>%
  rename(z2 = carreau) %>%
  select(-x,-y)
...
```
On voit pas grand chose si ce n'est que la grosse composante n'a pas tant de zones à risque ce qui doit être proche de l'idée de morcellement de la zone elle doit tenir sur quelques points faibles (peu d'intersection fragiles. (a pousser )
