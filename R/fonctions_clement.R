#' Cleans the initial dataset by removing rows that are not in the correct format
#'  and aggregates duplicated lines
#'
...
clean_init_dt <- function(input_dt) {
  ...
  nb_obs <- z1 <- z2 <- NULL
  dt <- copy(input_dt)
  dt <- dt[, .(nb_obs = sum(nb_obs)), by = .(z1, z2)]
  
  ## Suppress lines in the wrong format
  ## TODO: very specific to French use case
...
#' Return elements of z1 with associated connected components id
#' from a given z1 x z2 crossing data.table
#'
#' @param input_dt (data.table) containing the z1xz2 crossing data with nb_obs
#'
#' @return a data table where associating each z1 to the id of its connected
#' component
#'
#' @examples
#' input_dt <- toy_example_6
...
return_connected_components <- function(input_dt) {
  ...
  nodes <-
    data.table(from = names(clust$membership),
               id_comp = clust$membership)
...
#'
#' @param input_dt (data.table) containing the z1 x z2 crossing without
#' duplicated lines
#'
#' @return returns the crossover matrix (SparseMatrix), whose rows represent
#' the elements of z1 and the columns the elements of z2
#' (possibly aggregated if they contribute to the same connection)
#' and whose values are equal to the number of statistical units in the
#' considered z1 x z2 crossover
#'
#' @examples
#' input_dt <- toy_example_6
#' m_crois <- build_m_crois(input_dt)
#' @export
...
#' Prepare data , cleaning input data with z1xz2 crossing and extract lines
#' containing z2 which are not fully contained by one element of z1
#'
#' @inheritParams isolate_intersection
#' @return a list with the intersection on the first element and the lines
#' corresponding to the fully included square in the second element
#'
#' @examples
#' input_dt <- toy_example_6
#' prepared_data <- prepare_data(input_dt)
#' @export
...
#' @param input_dt (data.table) containing the z1 x z2 crossing with counts,
#' tabulation table
#'
#' @return a list with the intersection on the first element and the lines
#' corresponding to the fully included square in the second element
...
isolate_intersection <- function(input_dt) {
  ...
  out <-
    list(intersecting_z2 = dt_z2_multi_z1, fully_included_z2 = dt_z2_mono_z1)
  
  return(out)
}

#' Find at-risk-of-differenciation areas.
#' Build the crossover matrix defined in \code{build_m_crois}, and operates
#' the graph reduction functions on it and then returns the z1-zones
#' (union of elements of z1) at risk
#'
#' @inheritParams isolate_intersection
...
#' @param save_intermediate_data_file Character indicating the suffix of the
#' name of the saved intermediate results. If \code{NULL}, results are not
#' written on the hardware. The path root is taken from the working directory 
#' (getwd()). Else a `.rds` file is saved containing the initial crossing 
#' matrix, the simplified matrix after merging and the list of matrix after the
#' splitting operation.
#' @param simplify Boolean. If \code{TRUE} then the graph simplification
#' (merging + splitting) occurs. Otherwise the exhaustive search is directly
#' applied on the original graph.
#' @param verbose Boolean. If \code{TRUE} (default), the different steps of the
#' process are notified and progress bars provide an estimation of time left.
...
#'
#' @export
find_pbm_diff_tab <- function(input_dt,
  ...
  if (simplify) {
    #one can choose to skip these steps of graph reduction if desired
    
    if (verbose)
      message("< --- Merging method 1 --- > ")
    m_crois_1 <-
      agregate(m_crois,
               threshold,
               methode = "m1",
    ...
    if (verbose)
      message("< --- Merging methods 1 and 2 --- >")
    m_crois_2 <-
      agregate(m_crois_1,
               threshold,
               methode = "both",
  ...
  if (verbose)
    message("< --- Exhaustive search of differentiation problems --- >")
  l_ag <-
    search_diff_agregate(l_decomp, threshold, max_agregate_size)
...
#' From a given list of elements of z1, outputs all the information allowing to
#' evaluate if a differentiation problem exists on this zone 
#' (no information if no differneciation issue)
#'
#' @param list_z1  vector containing the elements of z1 constituting the area 
#' to be evaluated
#' @param input_dt (data.table) containing the triplets z1-z2-z1 corresponding 
#' to a connection between 2 elements of z1 throug one element of z2 with metadata
#' @param threshold (data.table) the frequency rule threshold  
#' below which a zone is considered at risk
#'
...
#' return_diff_info(list_z1,input_dt)
#'
#' @export
...
return_diff_info <- function(list_z1, input_dt, threshold = 11) {
  ...
  dt <- clean_init_dt(dt)
  # all of the z2 elements partially or fully included in the area defined by 
  #list_z1, (and not fully included in one element of z1, only crossing z1, z2 
  # elements are interesting here)
  z2_target <- dt[z1 %in% list_z1,]$z2
  
  # z2 elements crossing the list_z1 area and other elements of z1 than those 
  # in list_z1 (frontiers of the area)
  # z2 at the frontier of the area
  at_risk_crossing <-
    unique(dt[z2 %in% z2_target &  !z1 %in% list_z1, "z2"])
  
  # build the internal_diff_table  
  # (part of the intersection of at_risk_crossing inside the list_z1 area)
  # and the external_diff_table  
  # (part of the intersection of at_risk_crossing outside the list_z1 area)
  ...
  if (sum(internal_diff_table$nb_obs) < threshold)
    internal_diff_issue <- TRUE
  if (sum(external_diff_table$nb_obs) < threshold)
    external_diff_issue <- TRUE
...
#' output a leaflet interactive map from a \emph{situation}, corresponding to 
#' the set of information needed ton interpretn differencing issue s in a zone
#' built from z1 elements
#'
#' @param situation_table  a subset of crossing z1 x z2 with nb_obs 
#' at the intersection
#' @param geom_z1 the sf data.frame containing geometry of z1 elements
#' referenced in the situation table
#' @param geom_z2 the sf data.frame containing geometry of z2 elements 
#' referenced in the situation table
#' @param list_z1_to_color list of z1 elements whichh polygon will be colored 
#' in the output interactive map
#' @param list_z2_to_color list of z2 elements whichh polygon will be colored 
#' in the output interactive map
#' @param threshold Strictly positive integer indicating the confidentiality
#' threshold. z1 x z2 intersections which number of statistical units is under 
#' the threshold are colored in red
#' @param save_name boolean, if not nul the map is saved in the diffman_results
#'  with the given name
#'
...
draw_situation <-
  function(situation_table,
           geom_z1,
           geom_z2,
           list_z1_to_color = NULL,
           list_z2_to_color = NULL,
           threshold = 11,
           save_name = NULL) {
    nb_obs <- z2 <- z1 <- NULL
    ...
    inter_z2_z1 <- sf::st_intersection(geom_z1[, "z1"],
                                       geom_z2_inter[, "z2"])
    
    inter_z2_z1 <- merge(dt, inter_z2_z1, by = c("z1", "z2"))
    z1_fillColor <-
      with(geom_z1,
           ifelse(z1 %in% list_z1_to_color, "orange", "#3FC8FC"))
    z2_fillColor <- with(geom_z2, ifelse(
      z2 %in% list_z2_to_color,
      ifelse(nb_obs_z2 >= threshold, "#1FF064", "#FFD5C7"),
      "#04117A"
    ))
    z2_fillOpacity <-
      with(geom_z2, ifelse(z2 %in% list_z2_to_color, 1, 0))
    ...
    group_names <-
      c("z2 on two sides of one z1 area", "intersections", "inside z2")
    ...
    if (!is.null(save_name))
      htmlwidgets::saveWidget(m,
                              file = paste0(save_name, ".html"),
                              selfcontained = TRUE)
    
    m
  }

#' create the equivalent individual data.frame from a tabulate z1 x z2 
#' data.frame with integer (round in any case)
#'
...
create_fictive_ind_table <- function(tab_table) {
  ...
  tmp <- lapply(1:nrow(tab_table),
                function(i) {
                  # i <- 1
                  crossing <- tab_table[i,]
                  if (crossing$nb_obs == 0)
                    return(NULL)
...
#' @inheritParams find_pbm_diff_tab
#' 
#' @return a data.table containing the differentiation info for this component
...
one_component_risk_extraction <- function(input_dt,
  ...
  verbose  = FALSE
  if (length(unique(input_dt$z1)) > 1000)
    verbose = TRUE
  
  list_area_z1_at_risk <- find_pbm_diff_tab(
    input_dt,
    verbose = verbose,
    threshold = threshold,
  ...
  l_diff_info <- lapply(list_area_z1_at_risk,
                        function(list_z1)
                          return_diff_info(list_z1, dt, 11))
...
#' return the diff info from a input data table corresponding the whole graph 
#' which is a list of connected component of z1 perform the search of diff issue
#' and return the diff information for all the corresponding component.
#' 
#' @inheritParams one_component_risk_extraction
#' @param save_dir Character indicating the folder where intermediate results 
#' will be saved (useful if there is too many components)
#' 
#' @return a data.table containing the diff info for this component 
#' (same format than the return diff info function)
#'
#' @examples
#' input_dt <- toy_example_6
...
all_component_risk_extraction <-
  function(input_dt,
    ...
    #
    l_input_dt <-
      split(input_dt_wit_comp, input_dt_wit_comp$id_comp)
    ...
    extract_info_and_save <- function(i) {
      ...
      s <- Sys.time()
      message("work on component number ",
              i,
              " with ",
              nz1,
              " elements of z1 ",
    ...
    l_risk_compo <-
      lapply(seq_along(l_input_dt), extract_info_and_save)
    
    # in order to be able to link saved file name and z1 elements
    return(z1_to_component)
  }


#' Read diff_info info from files into a given directory and rbind them
#' @param save_dir the directory where the diff_info files 
#' (from all_component_risk_extraction) are included
#'
#' @return a data.table containing all the diff info for all related components
#'
#' @export
