...
one_component_risk_extraction <- function(input_dt){
  ...
  l_diff_info <- lapply(list_area_z1_at_risk,function(list_z1) return_diff_info(list_z1,input_dt,11))
...
all_component_risk_extraction <- function(input_dt){
  ...
  l_risk_compo <- lapply(l_input_dt,one_component_risk_extraction)
  
  out <- Reduce(l_risk_compo,rbind)
...
list_z1_compo <- compo[id_comp %in% c(1232,510,22),]$z1
input_dt <- data_rp[z1 %in% list_z1_compo]
res <- all_component_risk_extraction(input_dt)
```
