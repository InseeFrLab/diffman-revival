...
### Analysis of the big component reduction 

Here I will look on  the biggest component , try too cut it in part with influence mesure ?

```{r}
# mc cp s3/cguillo/res_grosse_composante.RDS res_grosse_composante.RDS

toy_example_6 <-
  data.table(
    z1 = c("M1","M1","M2","M2","M3","M3","M4","M5"),
    z2 = c("c1","c2","c2","c3","c1","c3","c4","c4"),
    nb_obs = c(17,15,8,2.9,20,17,14,3.5)
  )

usethis::use_data(toy_example_6)
```

## developp a function(which perform the whole extraction process) 
```{r}
## test
...
```
## TO DO :
- les fichiers portent le numéroi de la composantes et si je retournais les composantes utilisées à la fin
- difference between ::, require et library ?
- ajouter les requirements dans description
- vignette CI etc.. faire un point ave +Julien
- je pense abandonner le fait que la fonction retourne les diffi infos d'un coup (trop lourdingu avec 1900 componante, proposer une fonction de lecture des résultats -> + efficace, avec un Reduce)
