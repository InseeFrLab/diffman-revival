...
compo <- return_connected_components(data_rp)
compo[ ,.(n_com = length(z1)), by = "id_comp"]
input_dt <- data_rp[z1 %in% compo[id_comp == 1]$z1]
Sys.time()
...
```
- beginning : "2022-10-21 18:27:57 UTC"
- search diff : "2022-10-21 18:57:48 UTC" (8494 élément dans l_area at risk) (return diff_info trop lourde ?)
- diff_info : ?? Ok j'ai alléger diff info en arretant de préparer la table pour chaque list _z1 je la prépare en amont.. moins lourd iça prendra 30 min

## The whole risk extraction process with parallel workers ! 
```{r}
## test
...
```
## TO DO :
- ajouter les requirements dans description (foreach doParallel)
